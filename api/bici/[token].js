// /api/bici/[token] — fusiona crear + consulta en UN solo archivo/función.
// Motivo: el plan Hobby de Vercel tiene tope de 12 Serverless Functions;
// se agregó JOYÀ y el proyecto se pasó del límite (deploy con error
// "No more than 12 Serverless Functions can be added... Hobby plan").
// Mismo truco ya usado en api/joya/[token].js: como esta ruta es dinámica,
// también matchea literalmente /api/bici/crear — así que "crear" se trata
// como un token especial en vez de vivir en su propio archivo.
// NINGUNA lógica de negocio cambió: es el mismo código de
// api/bici/crear.js y el [token].js anterior, solo reorganizado.
//
// GET  /api/bici/<token>  → cupón (público, allowlist de campos)
// GET  /api/bici/crear    → smoke test (deps/env), sin uso de frontend
// POST /api/bici/crear    → crea la reserva y devuelve folio + token
//
// Body de POST: { idioma, duracionId, fecha, hora, cantidad, garantiaTipo,
//         nombre, email, telefono?, nacionalidad?, documento?, hotel?,
//         firma, aceptaTerminos, foto, fotoReserva?, hp }
// 200 → { folio, folioLabel, token, total, moneda }
// 409 → { error:'sin_disponibilidad', disponibles:N }
const { supa, leerJson, CAMPOS_PUBLICOS_BICI } = require('../_lib/supabase.js');
const { generarToken, esTokenValido } = require('../_lib/token.js');
const { ventana, hoyCancun, RE_FECHA, RE_HORA } = require('../_lib/fechas.js');
const { calcularTotal, IDIOMAS, folioLabel } = require('../_lib/catalogo-bicis.js');
...
async function obtenerCupon(req, res, token) {
  ...
  try {
    ...
    console.error('bici:obtenerCupon:', e.message);
